...
// ColorSelect.cpp: 애플리케이션에 대한 클래스 동작을 정의합니다.
//
...
BEGIN_MESSAGE_MAP(CColorSelectApp, CWinApp)
	...
	ON_COMMAND(ID_FILE_NEW, &CColorSelectApp::OnFileNew)
END_MESSAGE_MAP()
...
BOOL CColorSelectApp::InitInstance()
{
	...
	m_pMainWnd = pFrame;
	// 주 MDI 프레임 창을 만듭니다.
	if (!pFrame->LoadFrame(IDR_ColorSelectTYPE))
		return FALSE;
	// 공유 MDI 메뉴 및 액셀러레이터 키 테이블의 로드를 시도합니다.
	//TODO: 추가 멤버 변수를 추가하고 응용 프로그램에 필요한 추가 메뉴 형식에 대한
	//	호출을 로드합니다.
	HINSTANCE hInst = AfxGetResourceHandle();
	m_hMDIMenu  = ::LoadMenu(hInst, MAKEINTRESOURCE(IDR_ColorSelectTYPE));
	m_hMDIAccel = ::LoadAccelerators(hInst, MAKEINTRESOURCE(IDR_ColorSelectTYPE));
...
// CColorSelectApp 메시지 처리기

void CColorSelectApp::OnFileNew()
{
	CMainFrame* pFrame = STATIC_DOWNCAST(CMainFrame, m_pMainWnd);
	// 새 MDI 자식 창을 만듭니다.
	pFrame->CreateNewChild(
		RUNTIME_CLASS(CChildFrame), IDR_ColorSelectTYPE, m_hMDIMenu, m_hMDIAccel);
}
